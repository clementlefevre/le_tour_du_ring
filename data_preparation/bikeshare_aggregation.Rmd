...
groupy.per_date_only <- df %>% group_by(from_district_spatial_na,to_district_spatial_na,mode,hour_of_day,is.weekend,date_only) %>% summarise(total_counts=n())

ggplot(groupy.per_data %>% filter(from_district_spatial_na==101),aes(date_only,total_counts))+ geom_point()

groupy <- groupy.per_data %>% group_by(from_district_spatial_na,to_district_spatial_na,mode,hour_of_day,is.weekend) %>% summarise(median_trips_count=median(total_counts))
