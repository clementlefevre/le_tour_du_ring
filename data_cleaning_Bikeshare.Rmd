...
write.csv(df,'data/bikes_preprocessed_with_LOR.csv')
...
write.csv(df,'data/bikes_preprocessed_with_LOR_and_districts.csv')


```
# merge bikesharing trips with LOR and LOR Districts AND add status  duration
```{r}
df.with.LOR <- fread('data/bikes_preprocessed_with_LOR.csv') %>% select(-V1)
df.with.LOR.and.district <- fread('data/bikes_preprocessed_with_LOR_and_districts.csv') %>% select(-V1)
...
write.csv(df,'data/bikes_preprocessed_with_LOR_and_districts_and_duration.csv')
...
df.lor.districts <- df.lor.districts %>% select(spatial_na,spatial_al,PLRNAME,BZRNAME,PGRNAME,BEZNAME,longitude_LOR,latitude_LOR)
